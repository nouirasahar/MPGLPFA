...
def chatBot():
    """
    this function is for maram to discuss with the bot and test it, 
    maram can
        - choose use the handle_unkown_function in case she wants to teach the model by talking to it.
        - choose to handle unkown input by returing a `please provide more explanation` answer .  
    """
    knowledge_base = load_knowledge_base(filepath)

    print("ChatBot: Hello! Type your message or 'quit' to exit.")
    while True:
        user_input = input("You: ").strip()
        if user_input.lower() == "quit":
            break

        intent, matched_entry = find_best_match(user_input, knowledge_base)

        if matched_entry:
            response = random.choice(matched_entry["responses"])
            print(f"Bot: {response}")
        else:
            print("Bot : I'm sorry, I can't answer that, can you please provide more explanation? ")
            handle_unknown_input(user_input, knowledge_base, filepath)
            #can make a call to the `handle_unknown_input` function and teach the model the new entry



def getAnswer(userInput:str):
    print("Loading data.json from:", filepath)
    print("Knowledge base loaded")
    fallBackreply="I'm sorry, I can't answer that, can you please provide more explanation?"
    knowledge_base = load_knowledge_base(filepath)
    intent, matched_entry = find_best_match(userInput, knowledge_base)
    #print(f"Detected Intent : {intent}") # this is to learn the detected Intent from the user input. 
    if matched_entry:
        response = random.choice(matched_entry["responses"])
        return response
    else:
